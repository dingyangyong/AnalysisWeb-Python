...
var countlyView = Backbone.View.extend({
	...
	el: $('#content-container'),	//jquery element to render view into
...
window.ReportsView = countlyView.extend({
	el: $('#content-container'),
	template: null,
	templateData: {},
	initialize: function() {
		this.template = Handlebars.compile($("#template-reports").html());
	},
	...
	appChanged: function() {
		$("#report-app-name").val($("#sidebar-app-select").find(".text").text());
	},
	renderCommon: function(isRefresh) {		
		$(this.el).html(this.template(this.templateData));
		
		$("#add-page").tipsy({fallback: "Not available in the demo", gravity: 'e', offset: 5 });
		$("#create-report").tipsy({fallback: "Nope, this is not available either", gravity: 'e', offset: 5 });
		...
		$("#create-report").click(function() {
			...
			report.title = $("#report-app-name").val();
			
			$(".report-item:not(.report-case):visible").each(function() {
				report.pages[report.pages.length] = {
					"title": $(this).find(".title .text input").val(),
					"type": $(this).find(".cly-select .text").text(),
					"table": $(this).find(".option:nth-child(1) .checkbox").hasClass("checked"),
					"chart": $(this).find(".option:nth-child(2) .checkbox").hasClass("checked")
				}
			});
		});
		
		$("#add-page").click(function() {
			var backgroundPos = ["-147px", "-297px", "-445px"];
				newReportPage = $("#new-report-item").clone();

			newReportPage.find(".details>.image").css({"background-position": backgroundPos[Math.floor((Math.random()*3))]});
			$("#report-items").append(newReportPage.html());
		});
		...
		$("#report-date-done").click(function() {
			$("#date-picker").hide();
		});
		
		$("#report-items").sortable({ 
			containment: '#content-container',
			helper: function(e, el) {
				return el.clone().addClass("dragged");
			},
			cancel: '.report-case',
			items: '.report-item:not(.report-case)',
			tolerance: 'pointer',
			handle: '.sort-handle',
		...
		$("#drop-menu").droppable({ 
			...
			over: function(event, ui) {
				$(this).find(".message").text("DROP TO DELETE");
			},
			out: function(event, ui) {
				$(this).find(".message").text("DRAG THE PAGE HERE TO DELETE");
			}
		});
	},
	refresh: function() {
...
window.ManageAppsView = countlyView.extend({
	...
	renderCommon: function(isRefresh) {		
		...
		$("#clear-app-data").click(function() {
			CountlyHelpers.alert("There is no real data on this demo so consider it cleared.", "red");
		});
		
		$("#delete-app").click(function() {
			CountlyHelpers.alert("We need this app!", "red");
		});
		...
		$("#save-app-edit").click(function() {
			CountlyHelpers.alert("We didn't save your changes but at least you've seen how it feels.", "red");
		});
		...
		$("#save-app-add").click(function() {
			...
			CountlyHelpers.alert("Thanks for filling in all the details but we can't really create an application for you in the demo.", "red");
		});
	}
});
	...
	renderCommon: function(isRefresh) {
		...
		$.ajax({
			...
			success: function(users) {
				$('#content-container').html(self.template({
					users: users,
...
var AppRouter = Backbone.Router.extend({
	...
	reports: function() {
		this.renderWhenReady(this.reportsView);
	},
	events: function() {
	...
	renderWhenReady: function(viewName) { //all view renders end up here
		...
		if (this.readyToRender) {
			viewName.render();
			this.refreshActiveView = setInterval(function() { countlyDemo.realtime(); self.activeView.refresh(); }, 10000);
			this.pageScript();
		} else {
			$.when(countlyEvent.initialize(), countlySession.initialize(), countlyLocation.initialize(), countlyUser.initialize(), countlyDevice.initialize(), countlyCarrier.initialize(), countlyDeviceDetails.initialize(), countlyAppVersion.initialize()).then(function(){
	...
	initialize: function() { //initialize the dashboard, register helpers etc.
		...
		$(document).ready(function() {
			...
			$("#account-settings").click(function() {
				CountlyHelpers.alert("Settings of the user will appear here.", "black");
			});
		...
		$(document).bind("active.idleTimer", function(){
			...
			self.refreshActiveView = setInterval(function() { countlyDemo.realtime(); self.activeView.refresh(); }, 10000);
		});
	},

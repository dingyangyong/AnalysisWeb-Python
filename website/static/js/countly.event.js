(function(countlyEvent, $, undefined) {
	...
	countlyEvent.initialize = function() {
		if (!countlyCommon.DEBUG) {
			...
			_activeEvents = { "list": [ "section", "purchase", "score" ], "map": { "section": { "name": "Finished Sections", "count": "Times" }, "purchase": { "name": "In App Purchase", "count": "Number of purchases", "sum": "Total revenue (USD $)" }, "score": { "name": "User Score" } } };
			if (!_activeEvent && countlyEvent.getEvents()[0]) {
				_activeEvent = countlyEvent.getEvents()[0].key;
			}
		
			json = countlyDemo.eventsDb[_activeEvent];
	...
	countlyEvent.getActiveSegmentation = function() {
		return (_activeSegmentation)? _activeSegmentation : "No segmentation";
	}
	...
	countlyEvent.getEventData = function() {
		...
		var eventData = {},
			...
			countString = (eventMap[_activeEvent] && eventMap[_activeEvent].count)? eventMap[_activeEvent].count : "Count",
			sumString = (eventMap[_activeEvent] && eventMap[_activeEvent].sum)? eventMap[_activeEvent].sum : "Sum";
		...
		if (_activeSegmentation) {
			...
			eventData["tableColumns"] = ["Segmentation", countString];
			if (segmentsSum.length) {
			...
			eventData["tableColumns"] = ["Date", countString];
	...
	countlyEvent.getEventSummary = function() {
		...
		var eventMap = (_activeEvents)? ((_activeEvents.map)? _activeEvents.map : {}) : {},
			countString = (eventMap[_activeEvent] && eventMap[_activeEvent].count)? eventMap[_activeEvent].count.toUpperCase() : "COUNT",
			sumString = (eventMap[_activeEvent] && eventMap[_activeEvent].sum)? eventMap[_activeEvent].sum.toUpperCase() : "SUM";
